...
impl Templates {
    /// Read RON file into Templates struct
    pub fn load() -> Self {
        let file = File::open("resources/template.ron").expect("Template file should open");
        from_reader(file).expect("Templates should load")
    }
        ...
        if let Some(effects) = &template.provides {
            let _ = effects
                ...
                .for_each(|(provides, n)| match provides.as_str() {
                    ...
                    _ => eprintln!("Unknown entity effect: {}", provides),
                });
        };
    }
}
